...
```
###d).
Create the vector (4,6,3) and assign it the name temp
```{r}
tmp <- c(4,6,3) 
```
###e).
...
```
###g).
makes temp where there are 10 4's in a row, 20 6's and then 30 3's
```{r}
...
```
###c).
Finds the values in xVec which correspond to values in yVec
```{r}
x600<-xVec[yVec>600]
```
###d).
